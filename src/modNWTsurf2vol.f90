...
contains
    ...
    SUBROUTINE initHOSNWT(this, fileName, zMin, zMax, nZmin, nZmax, zMinRatio, zMaxRatio, isAddedMode)
        ...
        if (isFFTWinit.EQV..FALSE.) then
            Call Fourier_ini(this%nXmode_, this%nYmode_, this%nAddmode_, 3)
            isFFTWinit = .TRUE.
        end if
    ...
    subroutine init_read_mod(this)
        ...
        allocate(this%hosMode_%modeX(this%nXmode_, this%nYmode_),&
                 this%hosMode_%modeY(this%nXmode_, this%nYmode_),&
                 this%hosMode_%modeZ(this%nXmode_, this%nYmode_),&
                 this%hosMode_%modet(this%nXmode_, this%nYmode_),&
                 this%hosMode_%modeFS(this%nXmode_, this%nYmode_),&
                 this%hosMode_%modeFSt(this%nXmode_, this%nYmode_) )

        allocate(this%hosMode_%modeAdd(this%nAddmode_, this%nYmode_),&
                 this%hosMode_%modeAddt(this%nAddmode_, this%nYmode_) )
    ...
    subroutine buildGlobalMesh(this, zMin, zMax, nZmin, nZmax, zMinRatio, zMaxRatio)
        ...
        !!- build wave number -----------------
        allocate( this%hosMode_%kx(this%nXmode_) )
        allocate( this%hosMode_%ky(this%nYmode_) )
        allocate( this%hosMode_%kxy(this%nXmode_, this%nYmode_) )
        allocate( this%hosMode_%ktheta(this%nXmode_, this%nYmode_) )
        allocate( this%hosMode_%kxAdd(this%nAddmode_) )
    ...
    subroutine reconstructionFFTs(this)
        ...
        this%hosMesh_%nonDimEta = fourier_2_space(this%hosMode_%modeFS,'cos','cos')
        ...
        do iz = 1, this%hosMesh_%nZ
            ...
            ! Inverse FFTs
            this%hosMesh_%nonDimPhiX(:,:, iz) = fourier_2_space(this%hosMesh_%aVitX,'sin','cos')
            if (this%nYmode_.NE.1) then
                this%hosMesh_%nonDimPhiY(:,:, iz) = fourier_2_space(this%hosMesh_%aVitY,'cos','sin')
                ! this%hosMesh_%nonDimDvDt(:,:, iz) = fourier_2_space(this%hosMesh_%aDvDt,'cos','sin')
            else
                this%hosMesh_%nonDimPhiY(:,:, iz) = 0.0_rp
                ! this%hosMesh_%nonDimDvDt(:,:, iz) = 0.0_rp
            end if
            this%hosMesh_%nonDimPhiZ(:,:, iz) = fourier_2_space(this%hosMesh_%aVitZ,'cos','cos')
            this%hosMesh_%nonDimPhit(:,:, iz) = fourier_2_space(this%hosMesh_%aPhit,'cos','cos')
            ! this%hosMesh_%nonDimDuDt(:,:, iz) = fourier_2_space(this%hosMesh_%aDuDt,'cos','cos')
            ! this%hosMesh_%nonDimDwDt(:,:, iz) = fourier_2_space(this%hosMesh_%aDwDt,'cos','cos')
    ...
    subroutine reconstructionAddFFTs(this)
        ...
        do iz = 1, this%hosMesh_%nZ
            ...
            do ix = 1, this%hosMesh_%nX
                ...
                this%hosMesh_%nonDimAddPhiX(ix, :, iz) = fourier_2_space_y(this%hosMesh_%lVitX,'cos')
                this%hosMesh_%nonDimAddPhiY(ix, :, iz) = fourier_2_space_y(this%hosMesh_%lVitY,'sin')
                this%hosMesh_%nonDimAddPhiZ(ix, :, iz) = fourier_2_space_y(this%hosMesh_%lVitZ,'cos')
                this%hosMesh_%nonDimAddPhit(ix, :, iz) = fourier_2_space_y(this%hosMesh_%lPhit,'cos')

                ! this%hosMesh_%nonDimAddDuDt(ix, :, iz) = fourier_2_space_y(this%hosMesh_%lDuDt,'cos')
                ! this%hosMesh_%nonDimAddDvDt(ix, :, iz) = fourier_2_space_y(this%hosMesh_%lDvDt,'sin')
                ! this%hosMesh_%nonDimAddDwDt(ix, :, iz) = fourier_2_space_y(this%hosMesh_%lDwDt,'cos')
            enddo
