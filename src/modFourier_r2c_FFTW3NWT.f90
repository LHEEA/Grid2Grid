...
CONTAINS
!
SUBROUTINE Fourier_ini(modenX, modenY, modenAdd, library)
!
...
!
! Memory allocation
ALLOCATE(in(nX,nY), out(nX,nY), s_2_f(nX,nY), f_2_s(nX,nY))
ALLOCATE(in_sin(nX-2,nY), out_sin(nX-2,nY))
!
ALLOCATE(in_y(nY), out_y(nY), s_2_f_y(nY), f_2_s_y(nY))
!
ALLOCATE(in_add(nAdd,nY), out_add(nAdd,nY), s_2_f_add(nAdd,nY), f_2_s_add(nAdd,nY))
!
! Fourier transforms on 'dealiased' domain i.e. extended
...
SELECT CASE (lib)
   CASE (3)
      ...
      IF (nY == 1) THEN
         ! 1D FFTs
         CALL dfftw_plan_r2r_1d(plan_CC, nX,   in(1,1),     out(1,1),     FFTW_REDFT00, flag)
         CALL dfftw_plan_r2r_1d(plan_SC, nX-2, in_sin(1,1), out_sin(1,1), FFTW_RODFT00, flag)
         !
         CALL dfftw_plan_r2r_1d(plan_CC_add,   nAdd, in_add(1,1), out_add(1,1), FFTW_REDFT01, flag)
         CALL dfftw_plan_r2r_1d(plan_CC_add_I, nAdd, in_add(1,1), out_add(1,1), FFTW_REDFT10, flag)
         CALL dfftw_plan_r2r_1d(plan_SC_add,   nAdd, in_add(1,1), out_add(1,1), FFTW_RODFT01, flag)
         CALL dfftw_plan_r2r_1d(plan_SC_add_I, nAdd, in_add(1,1), out_add(1,1), FFTW_RODFT10, flag)
	 	 !
         ! Fourier transforms on 'dealiased' domain i.e. extended
         IF (nd1 > 2) THEN
      ...
      ELSE
         ! 2D FFTs
         CALL dfftw_plan_r2r_2d(plan_CC, nX,   nY,   in(1,1),     out(1,1),     FFTW_REDFT00, FFTW_REDFT00, flag)
         CALL dfftw_plan_r2r_2d(plan_SC, nX-2, nY,   in_sin(1,1), out_sin(1,1), FFTW_RODFT00, FFTW_REDFT00, flag)
         CALL dfftw_plan_r2r_2d(plan_CS, nX,   nY-2, in(1,2),     out(1,2),     FFTW_REDFT00, FFTW_RODFT00, flag)
         CALL dfftw_plan_r2r_2d(plan_SS, nX-2, nY-2, in_sin(1,2), out_sin(1,2), FFTW_RODFT00, FFTW_RODFT00, flag)
         ! 2D FFTs y-direction only
         CALL dfftw_plan_r2r_1d(plan_Cy, nY,   in_y(1), out_y(1), FFTW_REDFT00, flag)
         CALL dfftw_plan_r2r_1d(plan_Sy, nY-2, in_y(2), out_y(2), FFTW_RODFT00, flag)
         ! 2D FFTs additionnal potential
         CALL dfftw_plan_r2r_2d(plan_CC_add,   nAdd, nY,   in_add(1,1), out_add(1,1), FFTW_REDFT01, FFTW_REDFT00, flag)
         CALL dfftw_plan_r2r_2d(plan_CC_add_I, nAdd, nY,   in_add(1,1), out_add(1,1), FFTW_REDFT10, FFTW_REDFT00, flag)
         CALL dfftw_plan_r2r_2d(plan_SC_add,   nAdd, nY,   in_add(1,1), out_add(1,1), FFTW_RODFT01, FFTW_REDFT00, flag)
         CALL dfftw_plan_r2r_2d(plan_SC_add_I, nAdd, nY,   in_add(1,1), out_add(1,1), FFTW_RODFT10, FFTW_REDFT00, flag)
         CALL dfftw_plan_r2r_2d(plan_CS_add,   nAdd, nY-2, in_add(1,2), out_add(1,2), FFTW_REDFT01, FFTW_RODFT00, flag)
         CALL dfftw_plan_r2r_2d(plan_CS_add_I, nAdd, nY-2, in_add(1,2), out_add(1,2), FFTW_REDFT10, FFTW_RODFT00, flag)
         CALL dfftw_plan_r2r_2d(plan_SS_add,   nAdd, nY-2, in_add(1,2), out_add(1,2), FFTW_RODFT01, FFTW_RODFT00, flag)
         CALL dfftw_plan_r2r_2d(plan_SS_add_I, nAdd, nY-2, in_add(1,2), out_add(1,2), FFTW_RODFT10, FFTW_RODFT00, flag)
	 	 !
...
!  on the free surface
s_2_f           = 1.0_rp / REAL(MAX(1,2*(nX-1)), RP)
s_2_f(2:nX-1,:) = 2.0_rp * s_2_f(2:nX-1,:)
IF (nY /= 1) THEN
   s_2_f           = s_2_f / REAL(MAX(1,2*(nY-1)), RP)
   s_2_f(:,2:nY-1) = 2.0_rp * s_2_f(:,2:nY-1)
END IF
!
IF (nY /= 1) THEN
   s_2_f_y         = 1.0_rp / REAL(MAX(1,2*(nY-1)), RP)
   s_2_f_y(2:nY-1) = 2.0_rp * s_2_f_y(2:nY-1)
END IF
!
!  on the wavemaker
s_2_f_add           = 1.0_rp / REAL(nAdd, RP) ! it is 2/(2*n)=2/(2*nAdd)=1/nAdd in fact
IF (nY /= 1) THEN
   s_2_f_add           = s_2_f_add / REAL(MAX(1,2*(nY-1)), RP)
   s_2_f_add(:,2:nY-1) = 2.0_rp * s_2_f_add(:,2:nY-1)
END IF
!
...
!  on the free surface
f_2_s           = 1.0_rp
f_2_s(2:nX-1,:) = 0.5_rp * f_2_s(2:nX-1,:)
IF (nY /= 1) THEN
   f_2_s(:,2:nY-1) = 0.5_rp * f_2_s(:,2:nY-1)
END IF
!
IF (nY /= 1) THEN
   f_2_s_y         = 1.0_rp
   f_2_s_y(2:nY-1) = 0.5_rp * f_2_s_y(2:nY-1)
END IF
!
!  on the wavemaker
f_2_s_add = 0.5_rp
IF (nY /= 1) THEN
   f_2_s_add(:,2:nY-1) = 0.5_rp * f_2_s_add(:,2:nY-1)
END IF
!
! on the free surface 'dealiased' domain
f_2_s_big            = 1.0_rp
...
CHARACTER(LEN=*)           :: CS_x, CS_y
!
in   = f(1:nX,1:nY) * f_2_s
!
CALL execute(CS_x, CS_y)
!
IDCT_FFTW(1:nX,1:nY) = out
!
...
!
IF ((nY /= 1) .AND. (CS_y == 'sin')) THEN
   in(:,1)  = 0.0_rp
   in(:,nY) = 0.0_rp
END IF
!
IF (CS_x == 'sin') in_sin = in(2:nX-1,:)
!
...
!
in_add = f(1:nAdd,1:nY)
!
CALL execute_add(CS_z, CS_y, 1)
!
DCT_FFTW_add(1:nAdd,1:nY) = out_add * s_2_f_add
!
...
CHARACTER(LEN=*)               :: CS_z, CS_y
!
in_add = f(1:nAdd,1:nY) * f_2_s_add
!
CALL execute_add(CS_z, CS_y, -1)
!
IDCT_FFTW_add(1:nAdd,1:nY) = out_add
!
