...
private
    ...
    contains
        ...
        procedure, public :: initialize => initHOSOcean

        !!- read and compute flow
        procedure, public :: correct => correctHOSOcean
...
contains
    ...
    SUBROUTINE initHOSOcean(this, fileName, zMin, zMax, nZmin, nZmax, zMinRatio, zMaxRatio)
        ...
        if (isFFTWinit.EQV..FALSE.) then
            Call Fourier_ini(this%nXmode_, this%nYmode_,3)
            isFFTWinit = .TRUE.
        end if
    ...
    subroutine init_read_mod(this)
        ...
        allocate(this%hosMode_%modeX(this%nXo2p1_, this%nYmode_),&
                 this%hosMode_%modeY(this%nXo2p1_, this%nYmode_),&
                 this%hosMode_%modeZ(this%nXo2p1_, this%nYmode_),&
                 this%hosMode_%modet(this%nXo2p1_, this%nYmode_),&
                 this%hosMode_%modeFS(this%nXo2p1_, this%nYmode_),&
                 this%hosMode_%modeFSt(this%nXo2p1_, this%nYmode_) )
    ...
    subroutine buildGlobalMesh(this, zMin, zMax, nZmin, nZmax, zMinRatio, zMaxRatio)
        ...
        allocate( this%hosMode_%kx(this%nXo2p1_) )
        allocate( this%hosMode_%kyN2(this%nYmode_) )
        allocate( this%hosMode_%kxy(this%nXo2p1_, this%nYmode_) )
        allocate( this%hosMode_%ktheta(this%nXo2p1_, this%nYmode_) )

        allocate( this%hosMode_%ikx(this%nXo2p1_, this%nYmode_) )
        allocate( this%hosMode_%iky(this%nXo2p1_, this%nYmode_) )
    ...
    subroutine reconstructionFFTs(this)
        ...
        Call fourier_2_space(this%hosMode_%modeFS,this%hosMesh_%nonDimEta)
        ...
        do iz = 1, this%hosMesh_%nZ
            ...
            Call fourier_2_space( this%hosMesh_%aVitX, this%hosMesh_%nonDimPhiX(:,:, iz) )

            if (this%nYmode_.NE.1) then
                Call fourier_2_space( this%hosMesh_%aVitY, this%hosMesh_%nonDimPhiY(:,:, iz) )
                ! Call fourier_2_space( this%hosMesh_%aDvDt, this%hosMesh_%nonDimDvDt(:,:, iz) )
            else
                this%hosMesh_%nonDimPhiY(:,:, iz) = 0.0_rp
                ! this%hosMesh_%nonDimDvDt(:,:, iz) = 0.0_rp
            end if

            Call fourier_2_space( this%hosMesh_%aVitZ, this%hosMesh_%nonDimPhiZ(:,:, iz) )
            Call fourier_2_space( this%hosMesh_%aPhit, this%hosMesh_%nonDimPhit(:,:, iz) )

            ! Call fourier_2_space( this%hosMesh_%aDuDt, this%hosMesh_%nonDimDuDt(:,:, iz) )
            ! Call fourier_2_space( this%hosMesh_%aDwDt, this%hosMesh_%nonDimDwDt(:,:, iz) )
