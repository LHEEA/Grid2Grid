MODULE modReconstruction_ocean
!
    ...
    DO ii2 = 1, jmax-jmin+1
        ...
        DO i2=2,n2o2p1
            ...
            IF ((k_n2*(zvect(ii1,ii2)+depth_star).LT.50.).AND.(k_n2*depth_star.LT.50.)) THEN
                coeff = COSH(k_n2*(zvect(ii1,ii2)+depth_star))/COSH(k_n2*depth_star) !* EXP(iMaginary*ky_n2(i2)*y(ii2))
                coeff2= SINH(k_n2*(zvect(ii1,ii2)+depth_star))/SINH(k_n2*depth_star) !* EXP(iMaginary*ky_n2(i2)*y(ii2))
            ELSE
        ...
        DO i1=2,n1o2p1
            DO i2=1,n2
                k_n2 = SQRT(kx(i1)**2+ky_n2(i2)**2)
                IF ((k_n2*(zvect(ii1,ii2)+depth_star).LT.50.).AND.(k_n2*depth_star.LT.50.)) THEN
                    coeff = COSH(k_n2*(zvect(ii1,ii2)+depth_star))/COSH(k_n2*depth_star) * EXP(iMaginary*ky_n2(i2)*yvect(ii2))
                    coeff2= SINH(k_n2*(zvect(ii1,ii2)+depth_star))/SINH(k_n2*depth_star) * EXP(iMaginary*ky_n2(i2)*yvect(ii2))
                ELSE
                    coeff = EXP(k_n2*zvect(ii1,ii2)) * EXP(iMaginary*ky_n2(i2)*yvect(ii2))
                    coeff2= coeff
                ENDIF
                !
...
DO i2 = 1, n2o2p1
    ikx(1:MIN(N_der(1),n1o2p1),i2) = iMaginary * kx(1:MIN(N_der(1),n1o2p1))
ENDDO
! Last mode contains cos information only and must not be part of the differentiation.
IF (iseven(n1)) ikx(n1o2p1,:) = 0.0_cp
! negative ky
DO i2 = 2, n2o2p1
    ikx(1:MIN(N_der(1),n1o2p1),n2-i2+2) = iMaginary * kx(1:MIN(N_der(1),n1o2p1))
ENDDO
!
iky = 0.0_cp
! y-derivative on n1 points (i.e. n1o2p1 modes)
DO i1 = 1, n1o2p1
    iky(i1,1:MIN(N_der(2),n2o2p1)) = iMaginary * ky_n2(1:MIN(N_der(2),n2o2p1))
    ! negative ky
    DO i2 = 2, MIN(N_der(2), n2o2p1)
        iky(i1,n2-i2+2) = - iMaginary * ky_n2(i2)
    ENDDO
...
!
END MODULE modReconstruction_ocean
